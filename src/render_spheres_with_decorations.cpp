...
RT::canvas generate_noisy_texture(size_t, size_t);
...
RT::world create_shapes_world()
{
	...
	/// let's now try and texture-map a noise pattern on a sphere
	{
		...
		auto noisy_texture = std::make_shared<RT::uv_image>(generate_noisy_texture(640, 512));
...
/// ----------------------------------------------------------------------------
/// create a noisy-texture of canvas_xpixels X canvas_ypixels size
RT::canvas generate_noisy_texture(size_t canvas_xpixels, size_t canvas_ypixels)
{
	auto noisy_pattern = std::make_shared<RT::gradient_perlin_noise_pattern>(
		RT::color::RGB(0x19, 0x19, 0x70), RT::color::RGB(0xff, 0xff, 0xff));

	RT::perlin_noise_pattern n(noisy_pattern,    /// pattern
	                           distrib(rnd_gen), /// a new-random-number
	                           8);               /// octaves

	RT::canvas canvas_retval   = RT::canvas::create_binary(canvas_xpixels, canvas_ypixels);
